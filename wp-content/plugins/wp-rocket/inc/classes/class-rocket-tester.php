...
class Rocket_Tester {
    ...
    public static $api_url_alternative = 'guard.zhaket.org/api/';
	...
	public function admin_menu() {
		add_submenu_page(
			$this->parent_slug,
			__('Activate WP Rocket', $this->text_domain),
			__('Activate WP Rocket', $this->text_domain),
			'manage_options',
			$this->slug,
	...
	public function menu_content() {
        ...
        <div class="wrap wpmlr_wrap" data-show="<?php echo $data_show ?>">
            <h1><?php _e('Activate WP Rocket', $this->text_domain); ?></h1>
			<?php if( isset($now) && !empty($now) ): ?>
                ...
                <div class="current_license">
                    ...
                    <form action="#" method="post" class="register_version_form">
                        ...
                        <button class="button button-primary"><?php _e('Register', $this->text_domain); ?></button>
                        <div class="result">
     ...
     * If you want add an interrupt in your plugin or theme simply can use this static method: Rocket_Tester::is_activated
	...
	public static function get_license_code() {
		$private_session = get_option(self::$option_name);
		$now = json_decode(get_option($private_session));
		if( empty($now) ) {
			return false;
		} else {
			return base64_decode($now->starter);
		}
	}
	 ...
	 */
	public static function send_request( $method, $params= [], $server2 = false ) {
		$param_string = http_build_query($params);
        $api_url = $server2 ? self::$api_url_alternative : self::$api_url;

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_URL,
            $api_url.$method.'?'.$param_string
        );
        $content = curl_exec($ch);
        $error = curl_error($ch);

        if (!empty($error) && !$server2) {
