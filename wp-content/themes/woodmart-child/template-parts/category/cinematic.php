<?php
/**
 * Category showcase — Cinematic variant.
 * Full-bleed hero, filmstrip of standout images, sticky product toolbar.
 *
...
?>
<div class="silken-cat silken-cat--cinematic">

	<section class="silken-cat__hero"<?php echo $hero_image ? ' style="background-image:url(' . esc_url( $hero_image ) . ')"' : ''; ?>>
		<span class="silken-cat__hero-overlay" aria-hidden="true"></span>
		<div class="silken-cat__hero-inner">
			<span class="silken-cat__eyebrow"><?php esc_html_e( 'دسته‌بندی', 'woodmart-child' ); ?></span>
			<h1><?php echo esc_html( $term->name ); ?></h1>
			<?php if ( $term->description ) : ?>
				<p class="silken-cat__desc"><?php echo esc_html( $term->description ); ?></p>
			<?php endif; ?>
			<a href="#silken-cat-products" class="silken-cat__cta"><?php esc_html_e( 'مشاهده محصولات', 'woodmart-child' ); ?></a>
		</div>
	</section>

	<?php if ( count( $gallery_images ) > 1 ) : ?>
		<div class="silken-cat__filmstrip">
			<?php foreach ( array_slice( $gallery_images, 1 ) as $img ) : ?>
				<span class="silken-cat__filmstrip-item" style="background-image:url(<?php echo esc_url( $img ); ?>)"></span>
			<?php endforeach; ?>
		</div>
	<?php endif; ?>
